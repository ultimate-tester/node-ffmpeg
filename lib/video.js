...
module.exports = function(filePath, settings, infoConfiguration, infoFile) {
      ...
      if (options.video.hasOwnProperty('disabled')) {
        ...
        if (options.video.hasOwnProperty('size')) {
          ...
          if (newDimension.aspect != null) {
            this.addFilterComplex('scale=iw*sar:ih, pad=max(iw\\,ih*(' + newDimension.aspect.x + '/' + newDimension.aspect.y + ')):ow/(' + newDimension.aspect.x + '/' + newDimension.aspect.y + '):(ow-iw)/2:(oh-ih)/2' +
                (options.video.paddingColor != null ? ':' + options.video.paddingColor : ''));
  ...
  var execCommand = function(callback, folder) {
    ...
    var finalCommands = ['ffmpeg -i'].concat(inputs.map(utils.addQuotes).join(' -i ')).
    ...
    utils.exec(finalCommands, settings, function(error, stdout, stderr) {
      ...
      if (!error) {
        ...
        if (onlyDestinationFile) {
          ...
          for (var i in result) {
            result[i] = path.join(folder, result[i]);
          }
        }
      }
  ...
  this.fnExtractFrameToJPG = function(/* destinationFolder, settings, callback */) {
    ...
    if (settings.frame_rate != null && /^([\d|\/]+)$/.exec(settings.frame_rate) === null)
    ...
    if (newDimension.aspect != null) {
      this.addFilterComplex('scale=iw*sar:ih, pad=max(iw\\,ih*(' + newDimension.aspect.x + '/' + newDimension.aspect.y + ')):ow/(' + newDimension.aspect.x + '/' + newDimension.aspect.y + '):(ow-iw)/2:(oh-ih)/2' +
          (settings.padding_color != null ? ':' + settings.padding_color : ''));
      this.addCommand('-aspect', newDimension.aspect.string);
    }
  ...
  this.fnAddWatermark = function(watermarkPath /* newFilepath , settings, callback */) {
    ...
    if (newFilepath == null)
      newFilepath = path.join(path.dirname(this.file_path),
